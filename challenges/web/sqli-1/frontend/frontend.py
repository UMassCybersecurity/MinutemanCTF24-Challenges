...
class DBManager:
    ...
    def populate_db(self, database='user_db', host="db", user="root"):
        self.connection = mysql.connector.connect(
            user=user, 
            password='c4cc207db461462f8e22aa3ddc744b66',
            host=host, 
        ...
        self.cursor = self.connection.cursor()
        # self.cursor.execute("DROP USER IF EXISTS 'nonroot'@'%'")
        # self.cursor.execute("CREATE USER 'nonroot'@'%' IDENTIFIED BY 'nonroot'")
        # self.cursor.execute("GRANT SELECT, INSERT ON *.* TO 'nonroot'@'%'")
